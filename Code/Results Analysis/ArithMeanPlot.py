...
BigFrame = pd.read_csv('/ML-based-prediction-of-CR-related-genes/Data/CR-probabilities/Strongest models/Joint_Analysis.csv')
...
if normalised:
    lim = 1.02
    amProb = BigFrame['Average_Prob_Normalised']
    dProb = BigFrame['DIP_Prob_Normalised']
    gProb = BigFrame['DIP_Prob_Normalised']
# Original
else:
    lim = 1
    amProb = BigFrame['Average_Prob_Original']
    dProb = BigFrame['DIP_Prob_Original']
    gProb = BigFrame['DIP_Prob_Original']

ColBar = pd.DataFrame({'Label': Label, 'Test': Test, 'Class': Class, 'nsMean': amProb, 'nsGeom': gmProb, 'ProbDIP': dProb, 'ProbGO': gProb} )
...
i = i+1
#12
L[i] = ax.scatter(x = ColBar.loc[class_indices, 'ProbGO'], y =  ColBar.loc[class_indices, 'ProbDIP'],  c = snblue, edgecolor='k', alpha=1, linewidth=1, s = 40, zorder=2)  
nci = i
i = i+1
#13
L[i] = ax.scatter(x = ColBar.loc[~class_indices, 'ProbGO'], y =  ColBar.loc[~class_indices, 'ProbDIP'],  c = snred, edgecolor='k', alpha=1, linewidth=1, s = 40, zorder=2) 
ci=i
i = i+1

xx = ColBar.loc[class_indices, 'ProbGO']
yy =  ColBar.loc[class_indices, 'ProbDIP']
Ncorr = round(pearsonr(xx, yy)[0] , 2)
xx = ColBar.loc[~class_indices, 'ProbGO']
yy =  ColBar.loc[~class_indices, 'ProbDIP']
Ccorr = round(pearsonr(xx, yy)[0] , 2)
xx = ColBar.loc[:, 'ProbGO']
yy =  ColBar.loc[:, 'ProbDIP']
...
# TITLE
if j == 0:
    ax.set_title('Arithmetic mean of CR-probabilities\n\n', size = titlesize, weight = titleweight)
else:
    ax.set_title('Geometric mean of CR-probabilities\n', size = titlesize, weight = titleweight)

# LATERAL
ax.text(lim2 + labeltickdist-0.01, 0.5, 'CR-probabilities mean', rotation=270, va='center', c=grey, size = labelsidesize) # Gray color
...
# LEFT
ax.set_ylabel("{PathDIP, CAT}'s normalised CR-probabilities", size = labelmainsize)
...
if j == 0:
    ...
    ax.text(0.5, ydown - labeltickdist + 0.01 , 'CR-probabilities mean', rotation=0, ha='center', c=grey, size = labelsidesize) # Gray color
 
if j == 1:
    ax.set_xlabel("{Go terms, BRF}'s normalised CR-probabilities", size = labelmainsize)
...
if j == 0:
    ...
    ax.text(0.5, lim2 + labeltickdist + 0.01, "{Go terms, BRF}'s normalised CR-probabilities", rotation=0, ha='center', size = labelmainsize) # Gray color
...
legend1 = ax.legend([ L[ci],L[nci] ],['$Ageing_{CR}$', '$Ageing_{NotCR}$'], loc='upper left', framealpha=0.8, fontsize = 9, handletextpad=0.1)
...
plt.annotate('Pearson correlation:\n$Ageing_{CR}$: ' + str(Ccorr) + '\t$Ageing_{NotCR}$: ' + str(Ncorr) + '\t$Ageing$: ' + str(Acorr), xy=(0.5, -0.17), size=11, ha='center', va='top', bbox=dict(boxstyle='round', fc='w', ec="k"), xycoords=trans)
